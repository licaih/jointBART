...
RcppExport SEXP JointBart(
    SEXP _in,      //number of observations in training data, vector
    SEXP _ip,	  	//dimension of x, int
    SEXP _inp,		  //number of observations in test data, vecotr
    SEXP _ix,		  //x, train,  pxn (transposed so rows are contiguous in memory), list
    SEXP _iy,		  //y, train,  nx1, list
    SEXP _ixp,		  //x, test, pxnp (transposed so rows are contiguous in memory), list
    ...
    SEXP _itau, // sigma prior, vec
    SEXP _inu, //sigma prior, vec
    SEXP _ilambda, //The scale of the prior for the variance, vector
    SEXP _isigest, //he prior for the error variance, vector
    SEXP _iw, //Vector of weights which multiply the standard deviation. list
    SEXP _idart,// need to change
    SEXP _itheta,// need to change
    SEXP _iomega,//need to change
    SEXP _igrp, // variable indicate, 1:p
    SEXP _ia, //	Sparse parameter for Beta(a, b)
    SEXP _ib,
    SEXP _irho,// Sparse parameter:
    SEXP _iaug, // Whether data augmentation
    SEXP _inkeeptrain,
    SEXP _inkeeptest,
    SEXP _inkeeptestme,
...
)
{
  ...
  //return data structures (using Rcpp) LH: create inside list
...
#define TEDRAW(a, b) tedraw[a][b]
  ...
  )
  {
    ...
    for(size_t i=0;i<(nd+burn);i++) {
      ...
      if(i==(burn/2)&&dart) bm.startdart(); //LH start graph in the middle
      //draw bart
      bm.draw(svec,gen);
      //draw sigma
      rss=0.0;
